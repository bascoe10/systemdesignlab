...
5. Cache failure must degrade, not break: if cache nodes are unreachable,
   serve from the database. `sdl chaos kill-cache` is the acceptance test.
6. Instrument everything. If `sdl dashboard` stays dark, it didn't happen.
...
- Performance: `EXPECTED_METRICS.md` targets met under `sdl load`.
- Stretch: survive `sdl chaos kill-cache` with error rate < 1%.
